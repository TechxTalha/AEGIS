...
@Service
public class AntigravityAgentAdapter implements SpecializedAgentProtocol {
    ...
    @Override
    public String submitJob(AgentJobRequest request) throws Exception {
    ...
    @Override
    public AgentJobStatus getJobStatus(String jobId) throws Exception {
        return jobStatuses.getOrDefault(jobId, AgentJobStatus.FAILED);
    }

    @Override
    public AgentJobResult getJobResult(String jobId) throws Exception {
    ...
    private void simulateAgentWork(String jobId) {
        new Thread(() -> {
            try {
                ...
                result.setPayload(Map.of(
                    "changedFiles", java.util.List.of("src/main/Example.java"),
                    "diff", "+ new code added"
                ));
