...
@Service
public class InfrastructureAgentAdapter implements SpecializedAgentProtocol {
    private static final Logger logger = LoggerFactory.getLogger(InfrastructureAgentAdapter.class);
    ...
    public String submitJob(AgentJobRequest request) {
        ...
        logger.info("Submitting infra job {}. Objective: {}", jobId, request.getObjective());
    ...
    public void cancelJob(String jobId) {
        logger.info("Canceling infra job {}", jobId);
    ...
    private void simulateAgentWork(String jobId) {
        new Thread(() -> {
            try {
                ...
                result.setMessage("Infra changes completed");
                result.setPayload(Map.of("servicesRestarted", 1));
                
                jobResults.put(jobId, result);
                jobStatuses.put(jobId, AgentJobStatus.COMPLETED);
                logger.info("Infra Job {} completed", jobId);
